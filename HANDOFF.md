...
**조작**: 좌우 `A/D` 또는 `←/→` · 점프 `Space`(접지 시, 정점 3.5칸) · 사다리 등반 `W/S` 또는 `↑/↓`(사다리와 겹칠 때).
...
- **점프 높이 = 3.5칸(고정)**: `PlayerController.jumpHeightCells=3.5`(맵 jump_units 기준). 점프 속도는 중력에서 역산 → gravityScale 바꿔도 정점 3.5칸 유지. `moveSpeed 6 / climbSpeed 5`는 임시값.
