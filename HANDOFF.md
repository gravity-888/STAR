...
> 최종 갱신: 2026-07-20 (플레이테스트 편의 키 + stage3 프리즘 우회 해법 차단)
>
...
| # | 단계 | 완료 조건 |
|---|---|---|
| 1 | **엔딩 + 최소 UI** | stage4 클리어 시 `stageOrder[0]`으로 순환하는 것([`MapLoader.GoToNext`](Assets/Scripts/Level/MapLoader.cs)) 대신 엔딩 처리. 타이틀·일시정지·스테이지 표시. `Assets/UI` 빈 폴더 |
| 2 | **프리팹 seam 구축** | `MapLoader`에 오브젝트별(거울·프리즘·게이트·플레이어·발판…) 프리팹 슬롯 추가. **슬롯이 비면 현재 색 사각형으로 폴백** |
...
**아직 안 정해진 것 (1번 착수 전에 결정 필요)**
- **진행 저장/이어하기 넣을지** — 넣으면 저장 포맷·초기화 UI가 딸려온다. 4스테이지 데모 규모면 **없이 가도 무방**.
- **UI 범위** — 타이틀 유무, 일시정지 유무, 스테이지 표시 형태.
