...
**조작**: 좌우 `A/D` 또는 `←/→` · 점프 `Space`(접지 시) · 사다리 등반 `W/S` 또는 `↑/↓`(사다리와 겹칠 때).
...
- **수평 점프 거리 미정**: `PlayerController.moveSpeed/jumpSpeed`는 임시값(6/11/5). 플레이테스트로 튜닝 필요.
- **타일 콜라이더 이음새**: 지형/발판을 셀 단위 BoxCollider2D로 깔아, 빠르게 달릴 때 이음새에 걸릴 수 있음(2D 유령 정점). 필요 시 CompositeCollider2D로 병합.
...
1. **Phase 4 — 거울 회전**: 플레이어가 Q/E로 조준한 거울 22.5° 회전 → `Mirror.Rotate()` + `BeamTracer.Trace()` 재호출. (거울 회전 로직은 이미 준비됨) — 플레이어가 있으니 이제 "가까운 거울 선택 → 회전 → 빛 재추적" 연결이 다음 순서.
...
> ✅ **Phase 5 완료**: 플레이어 이동/점프/사다리 등반 + 지형·발판 솔리드 콜라이더. 신 Input System 사용(구 Input.GetKey는 런타임 예외).
