...
1. **데모 흐름 마감**: stage4 클리어 시 현재 `stageOrder[0]`으로 **순환**함([`MapLoader.GoToNext`](Assets/Scripts/Level/MapLoader.cs)) — 엔딩/클리어 처리 필요.
2. **최소 UI**: 타이틀·일시정지·현재 스테이지 표시. 지금은 UI가 전무(`Assets/UI` 빈 폴더).
3. **프리팹화**: 플레이스홀더 사각형 → 실제 스프라이트 프리팹. `Assets/Art`·`Prefabs`·`Audio` 전부 비어 있음.
4. **플레이어 튜닝**: `moveSpeed 6`/`climbSpeed 5`는 아직 임시값.
